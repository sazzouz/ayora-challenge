...
def model__update(
    *,
    ...
    def user_profile__update(*, user: User, data) -> User:
    ...
    Some important notes:
        ...
        - `fields` can support m2m fields, which are handled after
            the update on `instance`.
        - If `auto_updated_at` is True, we'll try bumping `updated_at`
            with the current timestmap.


    NOTE: from https://github.com/HackSoftware/Django-Styleguide
    """
    has_updated = False
    m2m_data = {}
    update_fields = []
    ...
    for field in fields:
        ...
        assert model_field is not None, f"{field} is not a valid field for {instance.__class__.__name__}."
        ...
        # Perform simple comparison as a fallback
